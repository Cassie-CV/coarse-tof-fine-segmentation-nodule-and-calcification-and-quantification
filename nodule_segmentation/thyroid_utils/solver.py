...
class Solver(object):
	def __init__(self, config, train_loader, valid_loader, test_loader):
		...
		self.unet_path = config.weight_pretrain_path
	...
	def train(self):
		...
		unet_path_best = os.path.join(self.model_path, 'best_unet_score.pkl')
		writer = SummaryWriter(log_dir=self.log_dir)


		# 断点继训练,看看是否有上一训练时候保存的最优模型
		print(self.unet_path, os.path.isfile(self.unet_path))
		if os.path.isfile(self.unet_path):#False:
			# Load the pretrained Encoder
			self.unet.load_state_dict(torch.load(self.unet_path))
			self.myprint('Successfully Loaded from %s'%(self.unet_path))
		...
		for epoch in range(self.num_epochs):
			...
			for i, sample in enumerate(self.train_loader):
				...
				# if GT.size()!=torch.Size([10, 1, 256, 256]):
				...
				loss = 0.0*loss_lovz + 0.8*loss_softdice + 0.3*loss_bi_BCE
			...
			if (epoch + 1) % self.val_step == 0:
				...
				if unet_score > best_unet_score:
					...
					torch.save(best_unet,unet_path_best)
			...
			if (epoch+1)% self.save_model_step == 0:
				...
				torch.save(save_unet,os.path.join(self.model_path, 'epoch%d_Testdice%.4f_IOU%.4f.pkl'%(epoch+1, DC,IOU)))
		...
		with torch.no_grad():
			for i, sample in enumerate(data_lodear):
				...
				for ii in range(SR.shape[0]):
					...
					tmp_index = images_path[ii].split('/')[-1]
					try:
						tmp_index = int(tmp_index[5:].split('.')[0][:])
					except:
						tmp_index = int(tmp_index.split('.')[0][:])

...
from thyroid_utils.solver import Solver as Solver_or
import pandas as pd
...
def main(config):
    ...
    save = pd.DataFrame(test_list_GT, columns=['id'])
    save.to_csv(config.result_path+'id_test.csv', index=False, header=True,
                encoding='utf-8-sig')  # index=False,header=False表示不保存行索引和列标题
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_size', type=int, default=256)  # 网络输入img的size, 即输入会被强制resize到这个大小 calcification
    ...
    parser.add_argument('--batch_size_test', type=int, default=30) #30
    ...
    parser.add_argument('--cuda_idx', type=int, default=7)
    parser.add_argument('--DataParallel', type=bool, default=False) ##

    # data-parameters
    parser.add_argument('--weight_pretrain_path', type=str, default='/home/gpu2/10t_disk/dcx_new/project/thyroid_nodule/code/TNSCUI/result/dataset_stage2_256_pretrain_cal/test5_1/models/best_unet_score.pkl')
    
    
    parser.add_argument('--filepath_img', type=str, default='/data/cal_data/image_filer_roi_exist_256')
    parser.add_argument('--filepath_mask', type=str, default='/data/cal_data/cal_mask_roi_exist_256')
    parser.add_argument('--csv_file', type=str, default='/data/cal_data/train_cal_revised.csv')
    parser.add_argument('--fold_K', type=int, default=5, help='folds number after divided')
    parser.add_argument('--fold_idx', type=int, default=1)

    # result&save
    parser.add_argument('--result_path', type=str, default='./result/cal_revised_roi_exist')
